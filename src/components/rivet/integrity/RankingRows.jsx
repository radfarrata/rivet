...
export default function RankingRows({ rows, reportsByModel, reportsReady = false }) {
  return <div className="divide-y divide-border rounded-xl border border-border overflow-hidden">{rows.map(row => <section key={row.modelId} className="bg-card p-4 space-y-2">
    <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{row.model || modelLabel(row.modelId)}</h3><p className="text-xs text-muted-foreground">Alias: {row.modelAlias} · {row.modelRevision || 'exact revision unavailable'} · {row.aliasStatus}</p></div><strong>{row.avg == null ? '—' : `${row.avg.toFixed(1)}/100`}</strong></div>
    ...
    {reportsByModel && <p className="text-xs text-muted-foreground">{!reportsReady ? 'Report availability not yet confirmed' : reportsByModel[row.modelId] ? <Link className="text-foreground underline" to={`/report?snapshot=${reportsByModel[row.modelId].snapshotHash}`}>View domain report · {new Date(reportsByModel[row.modelId].generatedAt).toLocaleDateString()}</Link> : 'No accessible domain report'}</p>}
