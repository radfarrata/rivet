...
export default function CapabilityEvidenceMap() {
  ...
  return <div className="dark text-foreground max-w-5xl mx-auto space-y-4">
    ...
    {q.isLoading ? <p role="status">Calculating evidence-backed statistics…</p> : q.isError ? <div role="alert"><p>Could not load rankings. No partial rankings are shown.</p><button onClick={() => q.refetch()} className="underline">Retry</button></div> : <>
      <p className="text-xs text-muted-foreground">{q.data.methodologyVersion} · {q.data.excludedLegacy} legacy/incomplete results excluded · {q.data.blockedOfficial} results not eligible for official publication</p>
      ...
      <details className="rounded-xl border bg-card p-4 text-sm"><summary className="cursor-pointer font-semibold">Methodology and limitations</summary><p className="mt-2 text-muted-foreground">{q.data.methodology}</p><p className="mt-2 text-xs">Computed {new Date(q.data.computedAt).toLocaleString()}. This is a live view, not a frozen publication snapshot. CI values describe this sample, not all real-world tasks.</p></details>
