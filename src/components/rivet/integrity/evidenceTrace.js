export default function evidenceTrace(data) {
  ...
  const d = data;
  const steps = [
    step('version', 'Task version & methodology', d.legacy ? 'Legacy result: original task version, execution configuration and methodology were not captured.' : { ...d.version, methodology: d.evidence.methodologyVersion, taskSnapshot: d.snapshot }),
    ...
    step('model', 'Requested model & version limits', d.legacy ? { model: d.result.model, revision: 'Unknown' } : { requested: d.evidence.modelId, resolution: d.evidence.modelResolution, providerRevision: d.evidence.providerRevision || 'Not exposed by integration', startedAt: d.evidence.startedAt, completedAt: d.evidence.completedAt }),
    ...
    step('audit', 'Provenance audit trail', { artifactHash: d.evidence?.artifactHash || 'Unavailable', events: d.events }, 'tool_result')
  ];
