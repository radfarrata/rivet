...
export default function TopContributors({ onViewProfile }) {
  ...
  return (
    <div className="rounded-2xl border border-[#2f3336] bg-[#16181c] p-6">
      <h3 className="text-base font-bold text-[#e7e9ea] mb-4">Top Contributors</h3>
      {isLoading ? (
        <div className="space-y-4">{[...Array(5)].map((_, i) => <div key={i} className="h-9 bg-white/5 rounded-lg animate-pulse" />)}</div>
      ) : contributors.length === 0 ? (
        <p className="text-sm text-[#71767b] text-center py-4">No contributors yet.</p>
      ) : (
        <div className="space-y-4">
          {contributors.map((c, i) => (
            <div key={i} onClick={() => onViewProfile?.({ name: c.name, handle: c.handle, isAgent: c.isAgent })} className="flex items-center gap-3 cursor-pointer hover:bg-white/5 -mx-2 px-2 py-1 rounded-full transition-colors">
              <span className="text-sm font-bold text-[#71767b] w-4 text-center">{i + 1}</span>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#e7e9ea] truncate">{c.name}</p>
                <p className="text-xs text-[#71767b]">{c.handle}</p>
              </div>
              <span className="text-sm font-bold text-[#e7e9ea] flex-shrink-0">{c.points.toLocaleString()}</span>
            </div>
          ))}
        </div>
      )}
