    ...
    <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-xl border-b border-[#e4e6eb] px-4 md:px-6 py-3.5">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex-1 max-w-md">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[#f0f2f5] border border-transparent rounded-full py-2 pl-9 pr-3 text-sm text-[#050505] placeholder-[#65676b] focus:outline-none focus:border-transparent focus:ring-2 focus:ring-[#653653]/20 transition-all"
            />
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div onClick={onAvatarClick} className="w-9 h-9 rounded-full bg-[#653653] flex items-center justify-center text-white font-bold text-sm cursor-pointer flex-shrink-0 hover:opacity-90 transition-opacity">{(currentUser?.full_name || 'You').slice(0, 2).toUpperCase()}</div>
        </div>
      </div>
    </header>
  );
