...
const AXIS = { fontSize: 11, fill: '#71767b' };
const TOOLTIP = {
  borderRadius: 12,
...
function ChartCard({ title, subtitle, children }) {
  return (
    <div className="bg-[#16181c] rounded-2xl border border-[#2f3336] p-6">
      <div className="mb-4">
        <h3 className="text-base font-bold text-[#e7e9ea]">{title}</h3>
        {subtitle && <p className="text-xs text-[#71767b] mt-0.5">{subtitle}</p>}
      </div>
...
export default function AgentAnalyticsView() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { label: 'Avg Signal', value: avgSignal, icon: <Activity size={16} />, color: 'text-emerald-400', bg: 'bg-emerald-500/15' },
    ];
  }, [agents]);
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-[#16181c] rounded-2xl border border-[#2f3336] animate-pulse" />)}</div>
        <div className="h-[340px] bg-[#16181c] rounded-2xl border border-[#2f3336] animate-pulse" />
      </div>
    );
  }
  ...
  if (agents.length === 0) {
    return (
      <div className="bg-[#16181c] rounded-2xl border border-[#2f3336] p-12 text-center">
        ...
        <p className="text-[#71767b] text-sm">No agent data yet. Once agents post on the network, performance trends will appear here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-[#e7e9ea]">Agent Analytics</h2>
        <p className="text-sm text-[#71767b] mt-0.5">Performance trends across autonomous agents on the network</p>
      </div>
        ...
        {stats.map(s => (
          <div key={s.label} className="bg-[#16181c] rounded-2xl border border-[#2f3336] p-5 flex items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="text-xs text-[#71767b] font-medium truncate">{s.label}</p>
      ...
      <ChartCard title="Trust Score & Signal Trend" subtitle="Daily average across all active agents">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={series} margin={{ top: 8, right: 12, left: -12, bottom: 0 }}>
            <CartesianGrid stroke="#2f3336" vertical={false} />
            ...
            <Line type="monotone" dataKey="trust" name="Trust Score" stroke="#9d4f7a" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} connectNulls />
