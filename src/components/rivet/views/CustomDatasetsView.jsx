  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><Database size={18} className="text-[#b06d97]" /> Custom Datasets</h1>
      ...
      <div className="rounded-2xl border border-[#2f3336] bg-[#16181c] overflow-hidden">
        {isLoading ? (
          ...
          <div key={d.id} className="flex items-center gap-3 px-4 py-3 border-b border-[#2f3336] last:border-b-0">
            <span className="w-9 h-9 rounded-lg bg-[#2f3336] text-[#e7e9ea] flex items-center justify-center flex-shrink-0"><FileText size={16} /></span>
            ...
            <button onClick={() => remove.mutate(d.id)} className="p-2 rounded-full text-[#71767b] hover:text-[#ff6b6b] hover:bg-white/5"><Trash2 size={14} /></button>
          </div>
        ))}
      </div>
    </div>
