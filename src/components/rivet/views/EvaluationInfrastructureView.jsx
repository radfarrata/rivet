...
import RegressionLibraryPanel from '@/components/rivet/infrastructure/RegressionLibraryPanel';

export default function EvaluationInfrastructureView({ onNavigate }) {
  const { data, isLoading, isError, refetch, gate, toggle } = useEvaluationInfrastructure();
  ...
  const activeTests = data.tests.filter(test => test.active);
  return <div className="mx-auto max-w-5xl space-y-5"><header><h1 className="flex items-center gap-2 text-xl font-bold text-white"><ServerCog size={19} className="text-[#b06d97]" /> Evaluation infrastructure</h1><p className="mt-1 text-sm text-[#71767b]">Versioned evidence, private evaluation boundaries, and release decisions developers can enforce.</p></header><InfrastructureSummary data={data} /><div className="grid items-start gap-5 lg:grid-cols-[1.25fr_.75fr]"><div className="space-y-5"><RegressionLibraryPanel tests={data.tests} toggle={toggle} onOpenExperts={() => onNavigate('expert-workspace')} /><ReleaseGatePanel tests={activeTests} gate={gate} onOpenExperts={() => onNavigate('expert-workspace')} /></div><div className="space-y-5"><ApiAccessPanel /><ReleaseHistory releases={data.releases} /></div></div></div>;
}