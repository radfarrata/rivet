...
function SummaryStats({ transactions }) {
  ...
  const stats = [
    { label: 'Total Earned', value: `+${earned.toLocaleString()}`, unit: 'pts', icon: <TrendingUp size={16} />, color: 'text-[#31a24c]', bg: 'bg-[#31a24c]/10' },
    { label: 'Total Withdrawn', value: `${withdrawn.toLocaleString()}`, unit: 'pts', icon: <ArrowDown size={16} />, color: 'text-red-600', bg: 'bg-red-100' },
    { label: 'Pending', value: `${pending.toLocaleString()}`, unit: 'pts', icon: <Clock size={16} />, color: 'text-amber-600', bg: 'bg-amber-100' },
  ];
      ...
      {stats.map(s => (
        <div key={s.label} className="bg-[#ffffff] rounded-2xl border border-[#e4e6eb] p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-[#050505]">{s.value} <span className="text-xs text-[#65676b] font-normal">{s.unit}</span></p>
          </div>
        </div>
      ))}
    </div>
...
function StatusBadge({ status }) {
  const styles = {
    completed: 'bg-[#31a24c]/10 text-[#31a24c]',
    pending: 'bg-amber-100 text-amber-600',
  };
  return <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${styles[status] || 'bg-[#f0f2f5] text-[#65676b]'}`}>{status}</span>;
}
    ...
    <tr className="hover:bg-[#f0f2f5] transition-colors">
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isEarned ? 'bg-[#31a24c]/10' : 'bg-red-100'}`}>
            {isEarned ? <CheckCircle2 size={14} className="text-[#31a24c]" /> : <ArrowUpRight size={14} className="text-red-600" />}
          </div>
          <span className="text-[#050505] font-medium">{t.description}</span>
        </div>
      </td>
      <td className="px-5 py-3.5 text-[#65676b] text-xs">{formatRelative(t.created_date)}</td>
      <td className="px-5 py-3.5"><StatusBadge status={t.status} /></td>
      <td className={`px-5 py-3.5 text-right font-bold ${isEarned ? 'text-[#31a24c]' : 'text-red-600'}`}>{isEarned ? '+' : ''}{t.amount} pts</td>
    </tr>
  );
}
...
function TransactionsTable() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-[#050505]">Transactions</h2>
        <p className="text-sm text-[#65676b] mt-0.5">Live history of all your earnings and withdrawals</p>
      </div>
      ...
      <div className="bg-[#ffffff] rounded-2xl border border-[#e4e6eb] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#e4e6eb] text-xs text-[#65676b] uppercase tracking-wide">
            ...
            <tbody className="divide-y divide-[#e4e6eb]">
              {isLoading ? (
                <tr><td colSpan={4} className="px-5 py-8 text-center text-[#65676b]">Loading transactions...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-8 text-center text-[#65676b]">No transactions match this filter.</td></tr>
              ) : (
    ...
    <div className="space-y-5">
      ...
      <div className="bg-[#ffffff] rounded-2xl border border-[#e4e6eb] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e4e6eb]">
          <h3 className="text-base font-bold text-[#050505]">Recent Payments</h3>
          <span className="text-xs text-[#65676b]">{transactions.length} total</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
            ...
            <tbody className="divide-y divide-[#e4e6eb]">
              {isLoading ? (
                <tr><td colSpan={4} className="px-5 py-6 text-center text-[#65676b]">Loading...</td></tr>
              ) : recent.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-6 text-center text-[#65676b]">No transactions yet.</td></tr>
              ) : (
...
function WithdrawForm({ currentUser }) {
  ...
  return (
    <div className="bg-[#ffffff] rounded-2xl border border-[#e4e6eb] p-6 max-w-lg">
      <h2 className="text-xl font-bold text-[#050505] mb-1">Withdraw Points</h2>
      <p className="text-sm text-[#65676b] mb-5">Available: <span className="font-bold text-[#050505]">{balance.toLocaleString()} pts</span> (≈ ${(balance / 100).toFixed(2)})</p>
...
function EscrowStatusBadge({ status }) {
  const styles = { held: 'bg-amber-100 text-amber-600', released: 'bg-[#31a24c]/10 text-[#31a24c]', refunded: 'bg-[#f0f2f5] text-[#65676b]' };
...
function EscrowView({ currentUser }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-[#050505]">Escrow</h2>
        <p className="text-sm text-[#65676b] mt-0.5">Bounty payments held until you verify a task is complete</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map(s => (
          <div key={s.label} className="bg-[#ffffff] rounded-2xl border border-[#e4e6eb] p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-[#65676b] font-medium">{s.label}</p>
              <p className="text-lg font-bold text-[#050505]">{s.value} {s.unit && <span className="text-xs text-[#65676b] font-normal">{s.unit}</span>}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#ffffff] rounded-2xl border border-[#e4e6eb] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e4e6eb]">
          <h3 className="text-base font-bold text-[#050505]">Escrow Ledger</h3>
          <span className="text-xs text-[#65676b]">{escrows.length} records</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
            ...
            <tbody className="divide-y divide-[#e4e6eb]">
              {isLoading ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-[#65676b]">Loading escrow...</td></tr>
              ) : escrows.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-[#65676b]">No escrow records yet.</td></tr>
              ) : escrows.map(e => {
                ...
                return (
                  <tr key={e.id} className="hover:bg-[#f0f2f5] transition-colors">
                    <td className="px-5 py-3.5"><span className="text-[#050505] font-medium">{e.taskTitle || 'Untitled'}</span></td>
                    <td className="px-5 py-3.5 text-[#65676b]">{isRequester ? 'Requester' : 'Resolver'}</td>
                    <td className="px-5 py-3.5"><EscrowStatusBadge status={e.status} /></td>
                    <td className="px-5 py-3.5 text-right font-bold text-[#050505]">{e.amount} pts</td>
                    <td className="px-5 py-3.5 text-right">
                      {canRelease && <button onClick={() => releaseEscrow.mutate({ escrow: e, post: task, currentUser })} disabled={releaseEscrow.isPending} className="text-xs font-semibold text-[#31a24c] hover:text-[#42b72a] disabled:opacity-50">Verify & Release</button>}
                      {canRefund && <button onClick={() => refundEscrow.mutate({ escrow: e, post: task, currentUser })} disabled={refundEscrow.isPending} className="inline-flex items-center gap-1 text-xs font-semibold text-[#65676b] hover:text-[#050505] disabled:opacity-50"><RotateCcw size={12} /> Refund</button>}
