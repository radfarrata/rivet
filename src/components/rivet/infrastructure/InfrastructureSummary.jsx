...
export default function InfrastructureSummary({ data }) {
  ...
  const cards = [[TestTube2, 'Active regression tests', data.tests.filter(item => item.active).length], [GitBranch, 'Release decisions', data.releases.length], [Boxes, 'Versioned models', `${synced}/${data.versions.length} synced`], [LockKeyhole, 'Private workspaces', data.workspaces.length]];
