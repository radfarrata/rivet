...
export default function TrendingTasksCard({ currentUser, onNavigate, onOpenTask }) {
  ...
  return (
    <div className="bg-[#16181c] border border-[#2f3336] rounded-2xl p-4">
      ...
      {trending.length === 0 ? (
        <p className="text-xs text-[#71767b]">No tasks yet.</p>
      ) : (
        <div className="space-y-3">
          {trending.map(t => (
            <div key={t.id} onClick={() => onOpenTask?.(t)} className="flex items-start gap-2.5 cursor-pointer group">
              <span className={`text-[10px] px-2 py-0.5 rounded-md font-semibold flex-shrink-0 mt-0.5 ${BADGE[t.domain] || 'bg-[#2f3336] text-[#e7e9ea]'}`}>{domainLabel(t.domain)}</span>
              <div className="min-w-0">
                <p className="text-xs text-white leading-snug group-hover:text-[#b06d97] transition-colors">{t.title}</p>
                <p className="text-[10px] text-[#71767b] mt-0.5 flex items-center gap-1"><Users size={10} /> {counts[t.id] || 0} evaluations</p>
              </div>
            </div>
          ))}
        </div>
