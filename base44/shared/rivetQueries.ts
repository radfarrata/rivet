...
export async function dataset(b,user) {
  ...
  const enriched=results.map(r=>{
    ...
    return {...r,rawResponse:r.rawResponse || '',evidenceStatus:!r.evidenceId?'legacy':complete?'complete':'incomplete',methodologyVersion:r.methodologyVersion || 'legacy / unknown',familyHash:v?.familyHash,cohortHash:v?.contentHash,contaminationRisk:v?.contaminationRisk || 'unchecked',modelResolution:e?.modelResolution || 'unknown',providerRevision:e?.providerRevision || null,officialEligible:officialEligible(r,e,v,rs,fs),finalScore:fs[0]?.finalScore};
  });
