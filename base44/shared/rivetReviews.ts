import {METHOD,taskAccess,requireValue,scan,validReview,audit,evidenceComplete} from './rivetCore.ts';
export async function reviewAction(b,user,p) {
  ...
  if(p.action==='submitReview'){
    ...
    const saved=await s.HumanEvaluation.create({taskId:r.taskId,resultId:r.id,modelId:r.modelId,model:r.model,domain:r.domain,evaluatorId:user.id,evaluatorName:user.full_name||'Verified reviewer',score:p.score,verdict:p.verdict,failureCategory:p.failureCategory||'none',notes:p.notes.trim(),evidenceId:r.evidenceId,credentialId:credential.id,credentialStatusAtReview:'verified',conflictDeclaration:p.conflictDeclaration.trim(),hasConflict:false,methodologyVersion:METHOD,reviewWeight:1,reviewProtocol:'non_blinded_observational'});
  ...
  const qualified=reviews.filter(validReview);requireValue(new Set(qualified.map(x=>x.evaluatorId)).size>=2,'Two distinct verified, conflict-free reviews are required.');
  ...
  const saved=await s.ExpertEvaluation.create({traceId:`evidence:${r.evidenceId}`,resultId:r.id,taskId:r.taskId,evidenceId:r.evidenceId,expertId:user.id,expertName:user.full_name||'Adjudicator',verdict:p.score>=80?'false_positive':'confirmed_failure',severity:'medium',failureType:'other',structuredFeedback:p.notes.trim(),finalScore:p.score,reviewIds:qualified.map(x=>x.id),credentialId:credential.id,conflictDeclaration:p.conflictDeclaration.trim(),methodologyVersion:METHOD});
  await audit(b,user,'final_adjudication',saved.id,task.id,{resultId:r.id,reviewIds:saved.reviewIds,finalScore:p.score});return saved;
}
